...
<x-breadcrumb :items="[
    ...
    ['label' => 'Appointment #' . $appointment->id, 'url' => route('appointments.show', $appointment)],
    ['label' => 'Reschedule'],
]" />

<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="mb-0"><i class="fas fa-calendar-alt me-2 text-warning"></i>Reschedule Appointment</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('appointments.update', $appointment) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <label for="appointment_time" class="form-label">New Time <span class="text-danger">*</span></label>
                ...
                <div class="col-md-4">
                    ...
                    <select id="type" name="type" class="form-select @error('type') is-invalid @enderror" required>
                        @foreach(['opd' => 'OPD', 'ipd' => 'IPD', 'emergency' => 'Emergency', 'follow_up' => 'Follow-up'] as $val => $label)
                        <option value="{{ $val }}" {{ old('type', $appointment->type?->value) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    <label for="reason" class="form-label">Reason for Rescheduling <span class="text-danger">*</span></label>
                    <textarea id="reason" name="reason" rows="2"
                              class="form-control @error('reason') is-invalid @enderror"
                              required minlength="10" placeholder="Please state the reason...">{{ old('reason') }}</textarea>
